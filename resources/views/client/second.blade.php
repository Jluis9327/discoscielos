...
@section('content')
    <section id="testimonials" class="section-bg wow ">
        <div class="container">
                        ...
                        <select class="form-control" id="xs" onchange="javascript:handleSelect(this)">
                            ...
                            @for ($i = 2; $i>=0 ; $i--)
                                @if ($presentation[$i]->Date==$date)
                                    <option value="{{url('/client/zone/secondlevel/'.$dni.'/day/'.$presentation[$i]->Date)}}" {{"selected"}}>{{ $presentation[$i]->Date}}</option>
                                @else
                                    <option value="{{url('/client/zone/secondlevel/'.$dni.'/day/'.$presentation[$i]->Date)}}">{{ $presentation[$i]->Date}}</option>
                                @endif
                            @endfor
                        </select>
                    </div>
                </div>
                <form id="formulario" action="{{url('/client/zone/secondlevel/'.$dni.'/day/'.$date)}}" method="POST" enctype="multipart/form-data">
                    {{csrf_field()}}
            ...
            <center><a href="{{url('/client/zone/'.$dni)}}"><i class="ion-ios-undo-outline" style="font-size: 350%"></i></a>
